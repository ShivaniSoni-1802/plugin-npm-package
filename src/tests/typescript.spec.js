...
describe('TypeScript', () => {
    ...
    describe('description to comment', () => {
      ...
      export type MyEnum =
        ...
        it('Should work with directives', async () => {
			...
			`);
            const result = await plugin(schema, [], { directiveArgumentAndInputFieldMappings: { AsNumber: 'number' } }, { outputFile: '' });
            expect(result.content).toBeSimilarStringTo(`
    ...
    `);
      ...
      `);
            const result = (await plugin(schema, [], { futureProofEnums: true, allowEnumStringTypes: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
        export enum MyEnum {
          A = 'A',
          B = 'B'
    ...
    describe('Directives', () => {
      ...
      `);
        ...
        it('Should use last directive override', async () => {
			...
			`);
            const result = await plugin(schema, [], { directiveArgumentAndInputFieldMappings: { AsNumber: 'number', AsString: 'AsString' } }, { outputFile: '' });
            expect(result.content).toBeSimilarStringTo(`
      export type MyInput = {
